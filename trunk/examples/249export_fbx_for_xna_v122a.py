#!BPY
"""
Name: 'Autodesk FBX (.fbx) for XNA with preferred choices'
Blender: 248
Group: 'Export'
Tooltip: 'Export to an ASCII Autodesk FBX suitable for XNA '
"""
__author__ = "Campbell Barton, Fritz@triplebgames.com and JCBDigger"
__url__ = ['www.blender.org', 'blenderartists.org', 'Games.DiscoverThat.co.uk']
__version__ = "122"

__bpydoc__ = """\
This script is an exporter to the FBX file format. 
Modified to better suit use in Microsoft XNA.

http://wiki.blender.org/index.php/Scripts/Manual/Export/autodesk_fbx

Other scripts to work with Blender to produce model for XNA are available at:
...
# Attempted to rotate the completed model to suit XNA however this always 
# resulted in the animations being contorted.  I eventually created other 
# scripts to help work Y up in Blender prior to this export.
# Therefore all I ended up with in this script was to:
# Set my preferred buttons as on.
# Tidy up code to suit v2.48 of Blender.
# March 2009
# --------------------------------------------------------------------------
...
# Only used for rotating lamps and cameras
mtx_x90		= RotationMatrix( 90, 3, 'x') # used only for lamps
...
; Created by Blender FBX Exporter for XNA
; ----------------------------------------------------

'''

# This func can be called with just the filename (see also line 2905)
def write(filename, batch_objects = None, \
	...
	if BATCH_ENABLE:
		...
		for data in data_seq: # scene or group
			...
			# Dont pass batch options since we already used them
	...
	class my_bone_class:
		...
		# we need this because cameras and lights modified rotations
...
'''FBXHeaderExtension:  {
	...
	def object_tx(ob, loc, matrix):
		...
		if isinstance(ob, Blender.Types.BoneType):
			...
			parent = ob.parent
			if parent:
				par_matrix = parent.matrix['ARMATURESPACE']
			...
			# now we have calculated our settings in relation to the parent set back ready for the next bit
			matrix =  ob.matrix['ARMATURESPACE']
			
			
		else:
			# This is bad because we need the parent relative matrix from the fbx parent (if we have one)
			...
			if matrix:
				...
				scale = 1,1,1
				rot = 0,0,0
				# the above is used for all dummy objects including blend_root
	...
	def write_object_tx(ob, loc, matrix):
		...
		'''
		loc, rot, scale, matrix, matrix_rot = object_tx(ob, loc, matrix)
	...
	def write_bone(my_bone):
		...
		poseMatrix = write_object_props(my_bone.blenBone)[3]
	...
	def write_material(matname, mat):
		...
		file.write('\n\t\t\tProperty: "EmissiveFactor", "double", "",%.4f' % mat_dif) # mat_emit in original fbx converter
		
		file.write('\n\t\t\tProperty: "AmbientColor", "ColorRGB", "",%.4f,%.4f,%.4f' % mat_colamb)
		file.write('\n\t\t\tProperty: "AmbientFactor", "double", "",%.4f' % mat_amb)
		file.write('\n\t\t\tProperty: "DiffuseColor", "ColorRGB", "",%.4f,%.4f,%.4f' % mat_cold)
		file.write('\n\t\t\tProperty: "DiffuseFactor", "double", "",%.4f' % mat_emit) # mat_dif in original fbx converter
	...
	def write_video(texname, tex):
		...
		# XNA keep textures and mesh in the same folder
	...
	def write_sub_deformer_skin(my_mesh, my_bone, weights):
		...
		a bit silly but dose not really matter
		'''
	...
	for ob_base in tmp_objects:
		for ob, mtx in BPyObject.getDerivedObjects(ob_base):
			...
			if tmp_ob_type == 'Camera':
				...
				if EXP_ARMATURE:
					# TODO - armatures do not work in dupligroups!
					...
					if EXP_MESH_APPLY_MOD:
						...
						if EXP_ARMATURE:
							...
							if orig_mesh.getVertGroupNames():
								ob.copy().link(me)
								# If new mesh has no vgroups we can try add if verts are the same
				...
				if me:
					...
					if EXP_ARMATURE:
						...
						# Note - Fixed in BPyObject but for now just copy the function because testers may not have up to date modules,
...
Objects:  {''')
	...
	# NOTE - c4d and motionbuilder do not need normalized weights, but deep-exploration 5 does and (max?) do.
...
Connections:  {''')
	...
	def fbx_time(t):
		# 0.5 + val is the same as rounding up.
				...
				for my_ob in ob_generic:
					...
					if ob_generic == ob_meshes and my_ob.fbxArm:
						...
						for TX_LAYER, TX_CHAN in enumerate('TRS'): # transform, rotate, scale
							...
							elif 	TX_CHAN=='R':	context_bone_anim_vecs = [mtx[0].toEuler()			for mtx in context_bone_anim_mats] # changed from original fbx exporter
							...
							for i in xrange(3):
								...
								if not ANIM_OPTIMIZE:
									...
									# last frame to first frame, missing 1 frame on either side.
...
def fbx_ui_write(filename):
	...
	# Make the matrix
	# Can change rotation just by changing the following GLOBAL_MATRIX
	# Some of the necessary functions may have been chnaged to make this compatible with XNA
...
def fbx_ui():
	...
	Draw.Label('FBX Exporter for XNA...', x+20,y+195, 200, 20)
	
	if not GLOBALS['BATCH_ENABLE'].val:
		Draw.BeginAlign()
		GLOBALS['EXP_OBS_SELECTED'] =	Draw.Toggle('Selected Objects',	EVENT_REDRAW, x+20,  y+170, 160, 20, GLOBALS['EXP_OBS_SELECTED'].val,	'Export selected objects on visible layers', do_obs_sel)
		GLOBALS['EXP_OBS_SCENE'] =		Draw.Toggle('Scene Objects',	EVENT_REDRAW, x+180,  y+170, 160, 20, GLOBALS['EXP_OBS_SCENE'].val,		'Export all objects in this scene', do_obs_sce)
		Draw.EndAlign()

	Draw.Label('Objects to include...', x+20,y+145, 200, 20)
	Draw.BeginAlign()
...
def write_ui():
	...
	# globals
	GLOBALS['EVENT'] = 2    # was EVENT_REDRAW changed from the original fbx exporter
	GLOBALS['MOUSE'] = [i/2 for i in Window.GetScreenSize()]
	GLOBALS['FILENAME'] = ''
	GLOBALS['EXP_OBS_SELECTED'] =			Draw.Create(0) # dont need 2 variables but just do this for clarity
	GLOBALS['EXP_OBS_SCENE'] =				Draw.Create(1) # JCB prefer scene (see also line 221)
	...
	GLOBALS['EXP_LAMP'] =					Draw.Create(0) # JCB prefer no lamps
	GLOBALS['EXP_CAMERA'] =					Draw.Create(0) # JCB prefer no cameras
	GLOBALS['EXP_EMPTY'] =					Draw.Create(1)
	GLOBALS['EXP_IMAGE_COPY'] =				Draw.Create(1) # JCB prefer copy images
	# animation opts
	...
	GLOBALS['ANIM_ACTION_ALL'] =			[Draw.Create(1), Draw.Create(0)] # JCB prefer all actions not just the current action
	...
	while GLOBALS['EVENT'] != EVENT_EXIT:
		...
		Draw.UIBlock(fbx_ui) # Original FBX exporter uses Draw.UIBlock(fbx_ui,0)
	
	
# Script entry point
if __name__ == '__main__':
	if not set:
		Draw.PupMenu('Error%t|A full install of python 2.5.x is needed to run this script.')
	else:
		write_ui()
